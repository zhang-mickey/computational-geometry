...
Sweep a horizontal line from top to bottom, partitioning the polygon into trapezoids whenever the sweep line encounters a vertex.

Each trapezoid is bounded by two non-horizontal edges (edges of P) and two horizontal segments

The decomposition ensures that any vertical line intersects at most one trapezoid.

Binary Search on y-Intervals :
Find the trapezoid whose y-interval contains q's y-coordinate using binary search on the trapezoidal map.

Check Inside Polygon :
Determine if q lies within the trapezoid and also inside the original polygon P by checking its position relative to the bounding edges of the trapezoid.
